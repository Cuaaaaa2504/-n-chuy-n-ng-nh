...
2. Chạy file SQL nếu database chưa được tạo.
3. Chạy backend và recommendation service bằng `npm run start:stack`.
...
1. Khởi động SQL Server.
2. Chạy backend bằng `npm run start:dev`.
...
| Thành phần | Địa chỉ | Kết quả mong đợi |
|---|---|---|
| Frontend | `http://localhost:5173` | Hiển thị giao diện CineHunt |
| Backend | `http://localhost:3000/api` | Hiển thị Swagger UI |
| Recommendation | `http://localhost:8000/health` | Trả về trạng thái service/model |
| Recommendation docs | `http://localhost:8000/docs` | Hiển thị FastAPI Swagger |
...
1. Kiểm tra backend có chạy ở cổng `3000`.
2. Mở `http://localhost:3000/api`.
3. Kiểm tra lỗi proxy trong terminal frontend.
4. Kiểm tra `VITE_API_PROXY_TARGET`.
5. Kiểm tra lỗi CORS trong Developer Tools.
