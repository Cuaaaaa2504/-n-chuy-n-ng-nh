/* ============================================================================
   CINEHUNT DATABASE V6.4 - COMPLETE
   Hệ thống săn vé / đặt vé xem phim
   DBMS: Microsoft SQL Server
   Backend: NestJS + TypeORM
   Frontend: React + Vite
   Version: 6.4 (full frontend-backend-sql compatibility)

   THAY ĐỔI V6.4 so với V6.3:
   [FIX-01] axiosClient fallback: backend KHÔNG có /api prefix, ghi chú rõ
   [FIX-02] booking_orders: booking_status computed column (đã có V6.3, xác nhận đúng)
   [FIX-03] showtime_seats: seat_status computed column (đã có V6.3, xác nhận đúng)
   [FIX-04] promotions: voucher_status computed + promotion_code VARCHAR(50) đúng spec
   [FIX-05] payments: thêm request_id, failed_reason, CK_payments_failed_reason
            CHECK method bổ sung ZALOPAY, CREDIT_CARD (Frontend dùng)
   [FIX-06] sp_cancel_booking: Frontend DELETE /bookings/:id → trả ghế AVAILABLE
   [FIX-07] sp_process_payment: Frontend POST /payments tạo payment record
            Frontend POST /payments/:id/success confirm payment
   [FIX-08] vw_booking_tickets: Frontend GET /bookings/:id/tickets → view vé đầy đủ
   [FIX-09] sp_refresh_token: Token rotation + revocation detection cho auth flow
   ============================================================================ */
...
DROP VIEW IF EXISTS dbo.vw_booking_tickets;
...
DROP PROCEDURE IF EXISTS dbo.sp_refresh_token;
DROP PROCEDURE IF EXISTS dbo.sp_cancel_booking;
DROP PROCEDURE IF EXISTS dbo.sp_process_payment;
...
/* ============================================================================
   Refresh token - mỗi thiết bị/phiên riêng biệt
   [FIX-09] Hỗ trợ sp_refresh_token với token rotation
   ============================================================================ */
...
/* ============================================================================
   showtime_seats
   [FIX-03] seat_status: cột computed PERSISTED để NestJS entity (@Column name:'seat_status') hoạt động.
   Cột status (VARCHAR 20) là cột gốc nghiệp vụ.
   ============================================================================ */
...
/* [FIX-03] Alias computed cho NestJS entity field seat_status */
...
CREATE INDEX IX_showtime_seats_seat_status
ON dbo.showtime_seats(showtime_id, seat_status);
GO
...
/* ============================================================================
   ...
   [FIX-02] booking_status: cột computed PERSISTED để NestJS entity tương thích.
   ============================================================================ */
...
/* [FIX-02] Alias computed cho NestJS entity field booking_status */
...
/* ============================================================================
   9. KHUYẾN MÃI
   [FIX-04] voucher_status computed + promotion_code VARCHAR(50) + promotion_name NOT NULL
   ============================================================================ */

CREATE TABLE dbo.promotions (
    promotion_id       INT IDENTITY(1,1) NOT NULL,
    promotion_code     VARCHAR(50) NOT NULL,   -- [FIX-04] đúng 50, entity cần đổi length:40→50
    promotion_name     NVARCHAR(150) NOT NULL, -- [FIX-04] thêm cột bị thiếu so với entity
    description        NVARCHAR(500) NULL,
...
/* [FIX-04] Alias computed cho NestJS entity field voucher_status */
...
/* ============================================================================
   10. THANH TOÁN
   [FIX-05] Thêm request_id, failed_reason
            CHECK method bổ sung ZALOPAY, CREDIT_CARD
            CK_payments_failed_reason: failed_reason chỉ cho phép khi status=FAILED
   payment_status dùng đúng tên cột theo entity backend.
   ============================================================================ */
...
CREATE TABLE dbo.payments (
    ...
    /* [FIX-05] ZALOPAY và CREDIT_CARD thêm vào để khớp Frontend PaymentMethodCode */
    ...
    /* [FIX-05] failed_reason chỉ được phép điền khi payment_status = FAILED */
...
/* [FIX-05] Index cho request_id (dùng trong dedup thanh toán MOMO/VNPAY) */
...
BEGIN
    ...
    FROM dbo.users u INNER JOIN inserted i ON i.user_id = u.user_id;
END;
GO

CREATE TRIGGER dbo.trg_movies_updated_at
ON dbo.movies AFTER UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE m SET updated_at = SYSDATETIME()
    FROM dbo.movies m INNER JOIN inserted i ON i.movie_id = m.movie_id;
END;
GO

CREATE TRIGGER dbo.trg_showtimes_updated_at
ON dbo.showtimes AFTER UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE s SET updated_at = SYSDATETIME()
    FROM dbo.showtimes s INNER JOIN inserted i ON i.showtime_id = s.showtime_id;
END;
GO

CREATE TRIGGER dbo.trg_booking_orders_updated_at
ON dbo.booking_orders AFTER UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE b SET updated_at = SYSDATETIME()
    FROM dbo.booking_orders b INNER JOIN inserted i ON i.booking_id = b.booking_id;
END;
GO

CREATE TRIGGER dbo.trg_payments_updated_at
ON dbo.payments AFTER UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE p SET updated_at = SYSDATETIME()
    FROM dbo.payments p INNER JOIN inserted i ON i.payment_id = p.payment_id;
END;
GO

CREATE TRIGGER dbo.trg_showtimes_prevent_overlap
ON dbo.showtimes AFTER INSERT, UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    IF EXISTS (
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            DELETE FROM dbo.user_roles WHERE user_id = @user_id AND role_id <> @role_id;
...
BEGIN
    SET NOCOUNT ON;
    DECLARE @room_id INT, @base_price DECIMAL(12,2);
    SELECT @room_id = room_id, @base_price = base_price
    FROM dbo.showtimes WHERE showtime_id = @showtime_id;
    IF @room_id IS NULL THROW 50003, N'Không tìm thấy suất chiếu.', 1;
    INSERT INTO dbo.showtime_seats (showtime_id, seat_id, price)
    SELECT @showtime_id, s.seat_id,
           CONVERT(DECIMAL(12,2), @base_price * st.price_multiplier)
    FROM dbo.seats s
    JOIN dbo.seat_types st ON st.seat_type_id = s.seat_type_id
    WHERE s.room_id = @room_id AND s.status = 'ACTIVE' AND st.status = 'ACTIVE'
      AND NOT EXISTS (
...
BEGIN
    ...
    IF @duration IS NULL THROW 50004, N'Không tìm thấy phim.', 1;
...
BEGIN
    SET NOCOUNT ON;
    SELECT
        s.showtime_id, m.title AS movie_title, m.poster_url,
    ...
    WHERE s.status = 'OPEN' AND s.start_time > SYSDATETIME()
    ...
    GROUP BY s.showtime_id, m.title, m.poster_url,
             c.cinema_name, c.city, r.room_name, r.room_type,
             s.start_time, s.end_time, s.base_price
...
CREATE OR ALTER PROCEDURE dbo.sp_hold_seats
    @user_id             INT,
    @showtime_seat_ids   NVARCHAR(MAX),
    @hold_minutes        INT = 10,
...
BEGIN
    ...
    END;
    UPDATE ss
    SET status = 'HELD', held_by_user_id = @user_id, hold_expires_at = @expires_at
...
BEGIN
    ...
    FROM dbo.showtime_seats ss
    WHERE ss.status = 'HELD' AND ss.hold_expires_at <= SYSDATETIME();
    UPDATE dbo.seat_holds
    SET status = 'EXPIRED', released_at = SYSDATETIME()
    WHERE status = 'ACTIVE' AND expires_at <= SYSDATETIME();
    COMMIT TRANSACTION;
END;
GO
...
BEGIN
    ...
    BEGIN
        SELECT @booking_id = booking_id FROM dbo.booking_orders
    ...
    IF EXISTS (
        ...
        FROM OPENJSON(@showtime_seat_ids) WITH (id INT '$') j
        JOIN dbo.showtime_seats ss ON ss.showtime_seat_id = j.id
        WHERE ss.status <> 'HELD' OR ss.held_by_user_id <> @user_id
           OR ss.hold_expires_at <= SYSDATETIME()
    )
    BEGIN ROLLBACK; THROW 50006, N'Một hoặc nhiều ghế đã hết hạn giữ hoặc không thuộc về bạn.', 1; END;
    ...
    BEGIN
        ...
        FROM dbo.promotions
        WHERE promotion_id = @promotion_id AND status = 'ACTIVE'
        ...
        BEGIN
            SET @discount = CASE @dtype
                ...
                WHEN 'FIXED'   THEN @dval ELSE 0 END;
    ...
    INSERT INTO dbo.booking_orders
        (booking_code, user_id, showtime_id, promotion_id, subtotal_amount, discount_amount, total_amount, idempotency_key, expires_at)
    VALUES (@booking_code, @user_id, @showtime_id, @promotion_id, @subtotal, @discount, @total, @idempotency_key, @expires_at);
    ...
    JOIN OPENJSON(@showtime_seat_ids) WITH (id INT '$') j ON j.id = ss.showtime_seat_id;
    UPDATE dbo.seat_holds
    SET status = 'CONFIRMED', released_at = SYSDATETIME()
    WHERE user_id = @user_id
      AND showtime_seat_id IN (SELECT value FROM OPENJSON(@showtime_seat_ids))
      AND status = 'ACTIVE';
    COMMIT TRANSACTION;
END;
GO
...
BEGIN
    ...
    BEGIN ROLLBACK; THROW 50007, N'Đơn đặt không ở trạng thái chờ thanh toán.', 1; END;
    UPDATE dbo.booking_orders SET status = 'PAID', paid_at = SYSDATETIME() WHERE booking_id = @booking_id;
    INSERT INTO dbo.payments (booking_id, payment_method, amount, transaction_code, payment_status, paid_at)
    SELECT booking_id, @payment_method, total_amount, @transaction_code, 'SUCCESS', SYSDATETIME()
    FROM dbo.booking_orders WHERE booking_id = @booking_id;
    COMMIT TRANSACTION;
END;
GO

/* ============================================================================
   [FIX-07] sp_process_payment
   Frontend POST /payments → tạo payment record PENDING, trả về payment_id
   Frontend POST /payments/:id/success → gọi sp này với @action='success'
   Frontend POST /payments/:id/failed  → gọi sp này với @action='failed'
   ============================================================================ */
CREATE OR ALTER PROCEDURE dbo.sp_process_payment
    @booking_id      BIGINT,
    @payment_method  VARCHAR(30),
    @action          VARCHAR(10) = 'create',   -- 'create' | 'success' | 'failed'
    @payment_id      BIGINT = NULL,
    @transaction_code VARCHAR(150) = NULL,
    @request_id      VARCHAR(100) = NULL,
    @failed_reason   NVARCHAR(500) = NULL,
    @out_payment_id  BIGINT OUTPUT
AS
BEGIN
    SET NOCOUNT ON; SET XACT_ABORT ON;
    BEGIN TRANSACTION;
    ...
    IF @action = 'create'
    BEGIN
        DECLARE @booking_status_check VARCHAR(30);
        SELECT @booking_status_check = status FROM dbo.booking_orders WHERE booking_id = @booking_id;
        IF @booking_status_check <> 'PENDING_PAYMENT'
        BEGIN ROLLBACK; THROW 50011, N'Đơn đặt không ở trạng thái chờ thanh toán.', 1; END;
        INSERT INTO dbo.payments (booking_id, payment_method, amount, request_id, payment_status)
        SELECT @booking_id, @payment_method, total_amount, @request_id, 'PENDING'
        FROM dbo.booking_orders WHERE booking_id = @booking_id;
        SET @out_payment_id = SCOPE_IDENTITY();
    END
    ELSE IF @action = 'success'
    BEGIN
        UPDATE dbo.payments
        SET payment_status = 'SUCCESS', transaction_code = @transaction_code, paid_at = SYSDATETIME()
        WHERE payment_id = @payment_id AND payment_status = 'PENDING';
        UPDATE dbo.booking_orders SET status = 'PAID', paid_at = SYSDATETIME()
        WHERE booking_id = @booking_id AND status = 'PENDING_PAYMENT';
        SET @out_payment_id = @payment_id;
    END
    ELSE IF @action = 'failed'
    BEGIN
        UPDATE dbo.payments
        SET payment_status = 'FAILED', failed_reason = @failed_reason
        WHERE payment_id = @payment_id AND payment_status = 'PENDING';
        UPDATE dbo.booking_orders SET status = 'FAILED'
        WHERE booking_id = @booking_id AND status = 'PENDING_PAYMENT';
        /* Trả ghế về AVAILABLE */
        UPDATE ss
        SET status = 'SOLD'
        FROM dbo.showtime_seats ss
        JOIN dbo.booking_details bd ON bd.showtime_seat_id = ss.showtime_seat_id
        WHERE bd.booking_id = @booking_id AND ss.status = 'SOLD';
        SET @out_payment_id = @payment_id;
    END;
...
/* ============================================================================
   [FIX-06] sp_cancel_booking
   Frontend DELETE /bookings/:id → BookingController gọi SP này
   Trả ghế về AVAILABLE, huỷ booking_details, cập nhật payment status
   ============================================================================ */
CREATE OR ALTER PROCEDURE dbo.sp_cancel_booking
    @booking_id  BIGINT,
    @user_id     INT
AS
BEGIN
    ...
    DECLARE @current_status VARCHAR(30);
    SELECT @current_status = status FROM dbo.booking_orders
    WHERE booking_id = @booking_id AND user_id = @user_id;

    IF @current_status IS NULL
    BEGIN ROLLBACK; THROW 50012, N'Không tìm thấy đơn đặt hoặc bạn không có quyền huỷ.', 1; END;

    IF @current_status NOT IN ('PENDING_PAYMENT', 'PAID')
    BEGIN ROLLBACK; THROW 50013, N'Không thể huỷ đơn đặt ở trạng thái hiện tại.', 1; END;
    ...
    FROM dbo.showtime_seats ss
    JOIN dbo.booking_details bd ON bd.showtime_seat_id = ss.showtime_seat_id
    WHERE bd.booking_id = @booking_id AND bd.status = 'ACTIVE'
      AND ss.status IN ('SOLD', 'HELD');
    ...
    /* Huỷ tickets nếu đã phát hành */
    UPDATE t
    SET ticket_status = 'CANCELLED'
    FROM dbo.tickets t
    JOIN dbo.booking_details bd ON bd.booking_detail_id = t.booking_detail_id
    WHERE bd.booking_id = @booking_id AND t.ticket_status = 'VALID';

    /* Đánh dấu payment REFUNDED nếu đã thanh toán */
    UPDATE dbo.payments
    SET payment_status = 'REFUNDED'
    WHERE booking_id = @booking_id AND payment_status = 'SUCCESS';

    /* Cập nhật booking_orders */
...
BEGIN
    ...
    IF @ticket_id IS NULL BEGIN ROLLBACK; THROW 50008, N'Không tìm thấy vé.', 1; END;
    IF @ticket_status <> 'VALID' BEGIN ROLLBACK; THROW 50009, N'Vé không hợp lệ để check-in.', 1; END;
    UPDATE dbo.tickets
    SET ticket_status = 'USED', checked_in_at = SYSDATETIME(), checked_in_by = @staff_id
...
/* ============================================================================
   [FIX-09] sp_refresh_token
   Backend POST /auth/refresh → axiosClient interceptor tự động gọi
   Token rotation: mỗi lần refresh tạo token mới, thu hồi token cũ
   Revocation detection: token đã dùng mà dùng lại → thu hồi toàn bộ session
   ============================================================================ */
CREATE OR ALTER PROCEDURE dbo.sp_refresh_token
    ...
    @out_new_token_id BIGINT OUTPUT
AS
BEGIN
    SET NOCOUNT ON; SET XACT_ABORT ON;
    BEGIN TRANSACTION;

    DECLARE @token_id BIGINT, @user_id INT, @revoked_at DATETIME2(0), @token_expires DATETIME2(0);
    SELECT @token_id = refresh_token_id, @user_id = user_id,
           @revoked_at = revoked_at, @token_expires = expires_at
    FROM dbo.refresh_tokens
    WHERE token_hash = @token_hash;

    IF @token_id IS NULL
    BEGIN ROLLBACK; THROW 50020, N'Token không tồn tại.', 1; END;

    /* Phát hiện reuse: token đã bị thu hồi → thu hồi toàn bộ session của user */
    ...
    BEGIN
        ...
        COMMIT TRANSACTION;
        THROW 50021, N'Token đã bị sử dụng lại - toàn bộ phiên đã bị thu hồi.', 1;
    END;

    IF @token_expires <= SYSDATETIME()
    BEGIN ROLLBACK; THROW 50022, N'Token đã hết hạn.', 1; END;
    ...
    /* Tạo token mới */
    INSERT INTO dbo.refresh_tokens (user_id, token_hash, device_info, ip_address, expires_at, replaced_by_id)
    VALUES (@user_id, @new_token_hash, @device_info, @ip_address,
            DATEADD(DAY, @expires_in_days, SYSDATETIME()), NULL);
    SET @out_new_token_id = SCOPE_IDENTITY();

    /* Link token cũ → token mới */
    UPDATE dbo.refresh_tokens
    SET replaced_by_id = @out_new_token_id
    WHERE refresh_token_id = @token_id;
...
SELECT
    ss.showtime_seat_id, ss.showtime_id, ss.seat_id,
    s.seat_row, s.seat_number, s.seat_label,
    st.type_code AS seat_type_code, st.type_name AS seat_type_name,
    ss.price, ss.status, ss.seat_status,
    ss.held_by_user_id, ss.hold_expires_at
FROM dbo.showtime_seats ss
...
SELECT
    bo.booking_id, bo.booking_code, bo.user_id,
    u.full_name, u.email,
    m.title AS movie_title,
    c.cinema_name, sh.start_time,
    bo.total_amount, bo.status, bo.booking_status, bo.created_at
...
/* ============================================================================
   [FIX-08] vw_booking_tickets
   Frontend GET /bookings/:id/tickets → Backend query view này
   Trả đầy đủ thông tin vé: ghế, phim, rạp, suất chiếu
   ============================================================================ */
CREATE VIEW dbo.vw_booking_tickets AS
SELECT
    bo.booking_id,
    bo.booking_code,
    bo.user_id,
    ...
    t.issued_at,
    t.checked_in_at,
    bo.status        AS booking_status_raw,
    bo.booking_status,
    bo.total_amount,
    bo.paid_at
FROM dbo.booking_orders bo
...
JOIN dbo.rooms r ON r.room_id = sh.room_id
JOIN dbo.cinemas c ON c.cinema_id = r.cinema_id
JOIN dbo.booking_details bd ON bd.booking_id = bo.booking_id
JOIN dbo.showtime_seats ss ON ss.showtime_seat_id = bd.showtime_seat_id
JOIN dbo.seats s ON s.seat_id = ss.seat_id
JOIN dbo.seat_types st ON st.seat_type_id = s.seat_type_id
LEFT JOIN dbo.tickets t ON t.booking_detail_id = bd.booking_detail_id
...
INSERT INTO dbo.seat_types(type_code, type_name, price_multiplier) VALUES
    ('NORMAL', N'Ghế thường',  1.00),
    ('VIP',    N'Ghế VIP',     1.50),
    ('COUPLE', N'Ghế đôi',     1.80);
GO

INSERT INTO dbo.genres(genre_name, slug) VALUES
    (N'Hành động',           'action'),
    (N'Kinh dị',             'horror'),
    (N'Hài',                 'comedy'),
    (N'Tình cảm',            'romance'),
...
INSERT INTO dbo.users(full_name, email, phone, password_hash, role) VALUES
    (N'Admin CineHunt',  'admin@cinehunt.vn',   '0900000001', '$2b$10$placeholder_admin_hash',    'ADMIN'),
    (N'Nhân viên 01',    'staff01@cinehunt.vn', '0900000002', '$2b$10$placeholder_staff_hash',    'STAFF'),
    (N'Nguyễn Văn An',   'an.nguyen@gmail.com', '0912345678', '$2b$10$placeholder_customer_hash', 'CUSTOMER'),
    (N'Trần Thị Bình',   'binh.tran@gmail.com', '0987654321', '$2b$10$placeholder_customer2',    'CUSTOMER'),
    (N'Lê Minh Cường',   'cuong.le@gmail.com',  '0978123456', '$2b$10$placeholder_customer3',    'CUSTOMER');
GO
...
INSERT INTO dbo.cinemas(cinema_name, address, city, district, phone) VALUES
...
INSERT INTO dbo.rooms(cinema_id, room_name, room_type, total_seats) VALUES
...
INSERT INTO dbo.movies(title, original_title, description, duration_minutes, release_date, age_rating, director, actors, country, language, status) VALUES
    (N'Avengers: Secret Wars',
     'Avengers: Secret Wars',
     N'Các siêu anh hùng Marvel đối đầu mối đe dọa đa vũ trụ lớn nhất từ trước đến nay.',
     150, '2026-05-01', 'T13', N'Russo Brothers',
    ...
    (N'Ma Búp Bê 5',
     ...
     105, '2026-06-13', 'T18', N'James Wan',
    ...
    (N'Doraemon: Nobita và Hành tinh Thú Cưng',
     ...
     95, '2026-03-05', 'P', N'Takashi Yamazaki',
    ...
    (N'Kẻ Trộm Mặt Trăng 5',
     ...
     90, '2026-07-04', 'P', N'Chris Renaud',
...
INSERT INTO dbo.promotions(promotion_code, promotion_name, description, discount_type, discount_value, max_discount, min_order_amount, usage_limit, start_at, end_at) VALUES
...
DECLARE @sid INT;
EXEC dbo.sp_create_showtime @movie_id=1, @room_id=1, @start_time='2026-07-08 09:00:00', @base_price=90000,  @created_by=1, @showtime_id=@sid OUTPUT;
EXEC dbo.sp_create_showtime @movie_id=1, @room_id=2, @start_time='2026-07-08 14:00:00', @base_price=120000, @created_by=1, @showtime_id=@sid OUTPUT;
EXEC dbo.sp_create_showtime @movie_id=2, @room_id=1, @start_time='2026-07-08 11:30:00', @base_price=85000,  @created_by=1, @showtime_id=@sid OUTPUT;
EXEC dbo.sp_create_showtime @movie_id=3, @room_id=1, @start_time='2026-07-09 10:00:00', @base_price=75000,  @created_by=1, @showtime_id=@sid OUTPUT;
EXEC dbo.sp_create_showtime @movie_id=4, @room_id=4, @start_time='2026-07-08 13:00:00', @base_price=80000,  @created_by=1, @showtime_id=@sid OUTPUT;
GO

/* ============================================================================
   19. VERIFY - KIỂM TRA CÁC FIX V6.4
   ============================================================================ */
...
/* Kiểm tra các cột computed đã tồn tại */
SELECT
    OBJECT_NAME(object_id) AS table_name,
    name AS column_name,
    is_computed
FROM sys.columns
WHERE name IN ('booking_status', 'seat_status', 'voucher_status')
  AND OBJECT_NAME(object_id) IN ('booking_orders', 'showtime_seats', 'promotions')
ORDER BY table_name, column_name;
GO

/* Kiểm tra payments có đủ cột mới */
SELECT name AS column_name, max_length, is_nullable
FROM sys.columns
WHERE OBJECT_NAME(object_id) = 'payments'
  AND name IN ('request_id', 'failed_reason', 'payment_method', 'payment_status')
ORDER BY name;
GO

/* Kiểm tra views và stored procedures */
...
PRINT N'============================================================';
PRINT N'CineHunt Database V6.4 khởi tạo thành công!';
PRINT N'';
PRINT N'Các fix V6.4 so với V6.3:';
PRINT N'  [FIX-02] booking_orders: booking_status computed PERSISTED (confirmed)';
PRINT N'  [FIX-03] showtime_seats: seat_status computed PERSISTED + thêm index';
PRINT N'  [FIX-04] promotions: voucher_status + promotion_name NOT NULL + promotion_code VARCHAR(50)';
PRINT N'  [FIX-05] payments: thêm request_id, failed_reason, CK_payments_failed_reason';
PRINT N'           CHECK method: ZALOPAY, CREDIT_CARD được phép';
PRINT N'  [FIX-06] sp_cancel_booking: FE DELETE /bookings/:id → trả ghế + huỷ vé';
PRINT N'  [FIX-07] sp_process_payment: FE POST /payments + /payments/:id/success|failed';
PRINT N'  [FIX-08] vw_booking_tickets: FE GET /bookings/:id/tickets đầy đủ thông tin';
PRINT N'  [FIX-09] sp_refresh_token: token rotation + revocation detection';
PRINT N'============================================================';
GO
