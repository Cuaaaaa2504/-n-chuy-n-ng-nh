...
export default function SeatBookingPage({ darkMode: darkModeProp = false }: Props) {
  const { id } = useParams();
  ...
  const [heldSeatCodes, setHeldSeatCodes] = useState<string[]>([]);

  useEffect(() => {
    setHeldSeatCodes(selectedSeats.map((s) => s.seatCode));
  }, [selectedSeats]);
